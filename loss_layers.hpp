...
/*******************************************************************************/
template <typename Dtype>
class TVL1LossLayer : public LossLayer<Dtype> {
 public:
  explicit TVL1LossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "TVL1Loss"; }  
  void GradientX(const Blob<Dtype> & imgs, Blob<Dtype> & gX);
  void GradientY(const Blob<Dtype> & imgs, Blob<Dtype> & gY);  
  void UpdateWeight(const Blob<Dtype> & diff_data, const Blob<Dtype> & img_dx, const Blob<Dtype> & img_dy);
...
//      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
  ...
  Blob<Dtype> diff_data_;		// 1st order gradient of data term
  Blob<Dtype> img_dx_;			// x-gradient of image
  Blob<Dtype> img_dy_;			// y-gradient of image
  Blob<Dtype> weight_data_;		// weight of data term
  Blob<Dtype> weight_tv_x_;		// x-weight of tv term
  Blob<Dtype> weight_tv_y_;		// y-weight of tv term
  
  int update_iter_, forward_count_;
  bool tv_show_, is_update_;
  Dtype lambda_, delta_;
};
/*******************************************************************************/
