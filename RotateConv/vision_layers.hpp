...
template <typename Dtype>
class RotateConvolutionLayer : public BaseConvolutionLayer<Dtype> {
 public:
  explicit RotateConvolutionLayer(const LayerParameter& param)
      : BaseConvolutionLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);	  
  virtual ~RotateConvolutionLayer();	  
  virtual inline const char* type() const { return "RotateConvolution"; }
 ...
 protected:
  ...
  void Rotate(const cv::Mat& src, const Dtype angle, cv::Mat& dst);
  void BlobToMat(const Blob<Dtype>& src, std::vector<std::vector<cv::Mat> >& dst);
  void MatToBlob(const std::vector<std::vector<cv::Mat> >& src, Blob<Dtype>& dst);
  void RotateFilters(const Dtype angle);  
  
  void Convolution_cpu(const vector<Blob<Dtype>*>& bottom, 
	  const vector<Blob<Dtype>*>& top, const Dtype* weight);  
  void MaxResponse_cpu(const vector<Blob<Dtype>*>& top);
  void MeanResponse_cpu(const vector<Blob<Dtype>*>& top);  
  
  void Convolution_gpu(const vector<Blob<Dtype>*>& bottom, 
	  const vector<Blob<Dtype>*>& top, const Dtype* weight);  
  void MaxResponse_gpu(const vector<Blob<Dtype>*>& top);
  void MeanResponse_gpu(const vector<Blob<Dtype>*>& top);     
  
  int num_rotate_, forward_count_, rotate_gap_;
  ...
  Blob<Dtype> weight_warp_blob_;
  std::vector<std::vector<Blob<Dtype>* > > top_rotate_;
  ConvolutionParameter_RotateMode rotate_mode_;
};
