...
void BaseConvolutionLayer<Dtype>::InitWeightBiasFromFile() {		
	Dtype* weight = (this->blobs_[0].get())->mutable_cpu_data();
	std::ifstream fs_reader;
	fs_reader.open(file_name_.c_str(), std::ifstream::binary);

	int dims[4], count;
	fs_reader.read((char *)(dims), 4*sizeof(int));

	count = dims[0]*dims[1]*dims[2]*dims[3];
	CHECK_GT(count, 0);
	
	double* weight_copy = new double[count];
	fs_reader.read((char *)(weight_copy), count*sizeof(double));

	for (int i = 0; i < count; i++)
		weight[i] = weight_copy[i];

	delete[] weight_copy;	
		
	if (bias_term_) {
		Dtype* bias = (this->blobs_[1].get())->mutable_cpu_data();
		double* bias_copy = new double[num_output_];
		fs_reader.read((char *)(bias_copy), num_output_*sizeof(float));
...
void BaseConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->blobs_.size() > 0) {
	...
	if (init_from_file_) {
		LOG(INFO) << "Initialize weight and bias from file!";
		InitWeightBiasFromFile();
	}
	else {
